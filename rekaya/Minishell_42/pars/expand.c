...
/*   Updated: 2025/08/11 03:38:38 by wel-mjiy         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		t_lexer **lexer)
{
	...
	tmp = malloc(ft_egdelen(str) * sizeof(char *));
	if (!tmp)
		return (0);
	i = 0;
	copy_str = *str;
	if (*str == '$' && ft_isalpha((*(str + 1))) != 1 && *(str + 1) != '?')
	{
...
char	*cleanup_argv(char **str, t_lexer **lexer, t_expand_var *expand)
{
	...
	newstr = NULL;
	if (*str == '$' && *(str + 1) != '?' && (*lexer)->q[expand->i] != 22)
	{
...
void	handle_normal_word(t_expand_var *expand, t_lexer **lexer, char **p)
{
	...
	while (**p)
	{
		...
		one[0] = **p;
		one[1] = '\0';
		expand->str = strjoin(expand->result, one);
		expand->i += 1;
...
void	search_comapre(t_env *env, t_lexer **lexer)
{
	...
	p = (*lexer)->content;
	while ((*lexer)->q[expand->i])
	{
		if ((*lexer)->q[expand->i] == 0 && (*lexer)->lenght_normal > 0)
